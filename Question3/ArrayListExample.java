...
public class ArrayListExample {
     ...
     * @param input        {@code ArrayList<T>} input arraylist
     * @return ans         {@code double} total area in double   
     */
    public static <T extends TwoDShape<T>>double total_area(ArrayList<T> input){
        double ans = 0;
        for(T ss : input) {
            ans+=ss.area();
        }
     ...
     * @param input        {@code ArrayList<T>} input arraylist
     * @return ans         {@code double} total perimeter in double  
     */
    public static <T extends Rectangle>double total_perimeter(ArrayList<T> input){
        double ans = 0;
        for(T ss : input) {
            ans+=ss.perimeter();
        }
     ...
     * @param input        {@code ArrayList<T>} input arraylist
     */
    public static <T extends GeometricShape<T>>void describe_all(ArrayList<T> input){
        int totalShapeNum = 0;
        for(T ss : input) {
            ss.describe();
            totalShapeNum++;
        }
        System.out.println("Total number of shapes: "+totalShapeNum);
    }
     ...
     *      {@code new Sphere(0.0);          }     
     * @param input        {@code ArrayList<GeometricShape>} input arraylist
     */
    public static void add_empties(ArrayList<GeometricShape> input){
        input.add(new Circle(0.0));
        input.add(new Cone(0.0, 0.0));
        input.add(new Rectangle(0.0, 0.0));
        input.add(new Sphere(0.0));
    }
     ...
     * @param input        {@code ArrayList<T>} input arraylist
     * @return temp         {@code ArrayList<T>} total area in double
     */
    // TODO: Assignment 7 Part 3 -- implement the "supersize_list" method
    public static <T extends GeometricShape<T>>ArrayList<T> supersize_list(ArrayList<T> input){
        ArrayList<T> temp = new ArrayList<>();
        for(T ss: input){
            temp.add(ss.supersize());
        }
        return temp;
    } 
